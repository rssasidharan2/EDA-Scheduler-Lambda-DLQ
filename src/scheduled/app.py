...
def lambda_handler(event, context):
    ...
    try:
        ...
        if simulate_failure:
            ...
            print('   1. EventBridge Scheduler retry (up to 3 times)')
            print('   2. After all retries fail → Event sent to SQS DLQ')
            print('   3. DLQ triggers Lambda processor via Event Source Mapping')
            print('   4. Lambda processor sends SNS notification')
            raise Exception('Simulated failure for testing DLQ and SNS notification flow')
        ...
        print('   → EventBridge Scheduler will retry this execution')
        print('   → After 3 failed retries, event goes to SQS DLQ')
        print('=' * 80 + '\n')
